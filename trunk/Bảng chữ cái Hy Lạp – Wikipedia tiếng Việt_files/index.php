            ...
            function openSettingsMenu(){
                    if( settingsMenu ) {
                            ...
                            settingsMenu = $("<form>").append(
                                    $("<button>").css("width","100%").html("Tắt Chú&nbsp;thích nổi").button().click(function(){toggleRT(0)}),
                                    $("<br>"),
                                    $("<small>").html("Để bật lại Chú&nbsp;thích nổi, nhấn chuột vào liên&nbsp;kết ở cuối trang."),
                                    $("<hr>"),
                                    $("<label>").html("Trì&nbsp;hoãn xuất&nbsp;hiện: &nbsp; ").attr("for", "RTDelay"),
                                    $("<input>").attr({"id":"RTDelay","type":"number","value":settings[1],step:50,min:0,max:5000}),
                                    $("<label>").html(" mili&nbsp;giây").attr("for", "RTDelay"),
                                    $("<br>"),
                                    $("<span>").html("Hiển&nbsp;thị chú&nbsp;thích khi: &nbsp; "),
                                    $("<input>").attr({"id":"RTActivateHover", "type":"radio", "name":"RTActivate", "checked":settings[2]==0&&"checked", "disabled":"ontouchstart" in document.documentElement&&"disabled"}),
                                    $("<label>").html(" rê&nbsp;chuột").attr("for", "RTActivateHover"),
                                    " ",
                                    $("<input>").attr({"id":"RTActivateClick", "type":"radio", "name":"RTActivate", "checked":settings[2]==1&&"checked"}),
                                    $("<label>").html(" nhấn&nbsp;chuột").attr("for", "RTActivateClick")
                            ).submit(function(e){e.preventDefault()}).dialog({modal:true,width:400,title:"Tùy chọn Chú thích nổi",buttons:{"Lưu tùy chọn":function(){
