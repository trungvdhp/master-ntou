...
#pragma region  Update memberships by GPU
		tmr_GPU.StartCounter();
		update_memberships_kernel_v2<<<num_blocks, block_size>>>
			(d_points, d_centroids,d_memberships, d_NNT, G.N, G.D, G.K, G.M, G.fuzzifier);
		//CudaCheckError();
		t1 = t1 + tmr_GPU.GetCounter();
#pragma endregion
...
__host__ double * GFKM_GPU_v4(FILE * f, GFKM & G, int block_size, int stop_iter)
{
#pragma region Declare common variables
	int i, j, k;
...
{
	if (mode == 1)
		return GFKM_GPU_v1(f, G, block_size, stop_iter);
	else if (mode == 2)
		return GFKM_GPU_v2(f, G, block_size, stop_iter);
	else if (mode == 3)
		return GFKM_GPU_v3(f, G, block_size, stop_iter);
	else
		return GFKM_GPU_v4(f, G, block_size, stop_iter);
}